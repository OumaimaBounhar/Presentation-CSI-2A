...
\begin{document}
    ...
    \begin{frame}
        ...
        \begin{columns}[T]
            ...
            \begin{column}{0.43\textwidth}
                ...
                \onslide<4->{\vspace{0.3cm}\\ \footnotesize Each class $\boldsymbol{\Phi}_i$ $\geq$ \textbf{200} such samples.}
            \end{column}
        \end{columns}
    \end{frame}
    ...
    \begin{frame}
        ...
        \onslide<4->{
        \footnotesize
        {\color{red!70!black}Red}: successive pilot search levels, each splitting the angular range in two. \\
        {\color{blue!70!black}Blue}: final $\mathrm N_c = 8$ narrow DFT communication beams $\boldsymbol{\Phi}_i$.
        }
    \end{frame}
    ...
    \begin{frame}
        ...
        \end{center}
        \footnotesize The theoretical i.i.d.\ Rayleigh BER (\emph{Without Sionna}) and the BER measured on Sionna's flat-fading channel realizations (\emph{With Sionna}) overlap exactly, for every modulation order, $N_t=1$, $N_r=64$ -- confirming Sionna's channel generation matches theory, before introducing the RIS.
    \end{frame}
